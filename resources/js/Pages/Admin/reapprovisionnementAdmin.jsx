...
const supprimerProduit = async (index) => {
    if (index === null || index < 0 || index >= updates.length) return;

    const produit = updates[index];

    // Vérifier la balise meta CSRF
    const csrfMeta = document.querySelector('meta[name="csrf-token"]');
    const csrfToken = csrfMeta ? csrfMeta.content : null;
    if (!csrfToken) {
        alert("Token CSRF introuvable. Veuillez vérifier que la balise <meta name='csrf-token'> est présente dans votre HTML.");
        return;
    }

    try {
        const response = await fetch(`/reapprovisionnements/${produit.Id}`, {
            method: 'DELETE',
            headers: {
        ...
        // Supprimer du frontend seulement si la suppression serveur réussit
        setUpdates(prev => prev.filter((_, i) => i !== index));
        setShowConfirm(false);
        setProduitASupprimer(null);
        alert('Réapprovisionnement supprimé avec succès');
